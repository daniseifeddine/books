...
@section('content')
    <div class="container" style="margin-top:120px">
        <h1 class="text-4xl mb-3 page-title pb-5">ARABIC POETRY
        </h1>
    </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 md:grid-cols-4 sm:grid-cols-3 gap-5 p-2" id="ajax_search_result">
            ...
            @foreach ($books as $book)
                <div class="all p-2 rounded" id="">
                    <div>
                        <a class="book-name" href="{{ route('single_book', $book->name) }}"><img class="book-cover col-span-1"
                                src="{{ asset('books_cover/' . $book->cover_image) }}" alt="">
                            </img></a>
                    </div>
                    <div>
                        <p class="text-sm lg:text-lg font-bold text-center md:text-lg sm:text-base pt-2 p-1">
                            {{ $book->name }}</p>
                    </div>
                    <div class="line"></div>
                    <div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#searchQuery').on('keyup', function() {
                ...
                var categoryName = "Arabic-Poetry";
